...
class Stops2POIs:
    ...
    def find_poi(self, batch=None, radius=300):
        ...
        # Randomly select from nearby POIs
        ...
        gdf_stops.loc[gdf_stops.poi_num > 0, 'osm_id'] = [self.gdf.loc[x[random.randint(0, len(x) - 1)], 'osm_id']
                                                          for x in ind if len(x) > 0]
        gdf_stops.loc[gdf_stops.poi_num > 0, 'dist'] = [x[random.randint(0, len(x) - 1)]
                                                        for x in dist if len(x) > 0]
        ...
        self.data.to_parquet(os.path.join(ROOT_dir, f'dbs/stop_combined2poi/stops_poi_{batch}.parquet'), index=False)
