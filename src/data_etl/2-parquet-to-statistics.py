...
class StatsCompute:
    ...
    def load_data(self):
        ...
        df = spark.read.parquet(*self.file_paths_list[self.batch]).\
    ...
    def compute_group(self, grp=None):
        ...
        df_g = self.df.filter(self.df["grp"] == grp)
        self.data_stats_compute(data=df_g.toPandas(), batch=self.batch, grp=grp)
        end = time.time()
        print(f"Data {self.batch}-{grp} saved in {(end - start) / 60} minutes.")


if __name__ == '__main__':
    sc = StatsCompute(num_grps=20, batch=1)
    sc.load_data()
